import {
  ...
  const Laundry: React.FC = () => {
    ...
    const sendToBackend = async (amount: number): Promise<boolean> => {
      try {
        const response = await fetch('https://smartloansbackend.azurewebsites.net/laundry', { // 🔹 Ruta adaptada
          method: 'POST',
          headers: {
    ...
    const fetchAllLaundry = async () => {
      try {
        const response = await fetch('https://smartloansbackend.azurewebsites.net/all_laundry'); // 🔹 Ruta adaptada
        if (!response.ok) {
            ...
            <IonRow className="ion-justify-content-center">
              <IonCol sizeMd="6" sizeLg="4" sizeXs="12">
                <IonItem className="cash-item" lines="none">
                  <IonLabel position="floating" className="cash-label">Ingrese el pago</IonLabel>
                  <br />
                  <IonInput
                    type="number"
                    value={cash}
